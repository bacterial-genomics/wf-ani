...
def parseArgs():
    parser = ArgumentParser(
        description="Computes the average nucleotide "
    ...
    opt.add_argument(
        ...
        metavar="FLOAT",
        default=30.0,
        help="minimum fragment alignment " "identity [30.0]",
    )
    opt.add_argument(
        "-l",
        "--length",
        ...
        metavar="INT",
        default=0,
        help="minimum fragment alignment length " "(incl gaps) [0]",
    )
    opt.add_argument(
        "-o",
        "--outpath",
        ...
        metavar="PATH",
        default=None,
        help="output directory [ANI--<date>_<time>]",
    )
    opt.add_argument(
        "-s",
        "--step-size",
        type=require_int_positive,
        metavar="INT",
        default=200,
        help="number of bases to shift during "
    ...
    opt.add_argument(
        ...
        metavar="INT",
        default=1000,
        help="fragment lengths to slice " "nucleotides into prior to alignments [1000]",
    )
    opt.add_argument(
        "--bed",
        default=False,
    ...
    opt.add_argument(
        ...
        metavar="INT",
        default=3,
        help="precision decimal points to round " "ANI values [3]",
    )
    opt.add_argument(
        "--fill",
        type=str,
    ...
    opt.add_argument(
        ...
        metavar="FLOAT",
        default=0.97,
        help="minimum fraction of ACGT in each input set [0.97]",
    )
    opt.add_argument(
        "--min-aln-frac",
        type=require_float_0_to_1,
...
def genbank_to_fasta(infile, outdir):
    ...
    for rec in SeqIO.parse(infile, "genbank"):
        if float(GC(rec.seq)) == 0:
            sys.stderr.write("ERROR: {} appears to lack nucleotides".format(rec.name))
            sys.exit(1)
        records.append(rec)
...
def get_seqlen_and_filter_ambiguous(infile, min_ACGT):
    ...
    if ACGT_fraction < min_ACGT:
        sys.stderr.write(
            "ERROR: low composition ({:.1f}%) of unambiguous "
            "nucleotides in {}\n".format(ACGT_fraction * 100, os.path.basename(infile))
        )
        sys.exit(1)
    if ACGT_fraction != 1.0:
        sys.stderr.write(
...
def main():
    ...
    if not opts.refilter:
        ...
        gsize = (
            get_seqlen_and_filter_ambiguous(set1, opts.min_ACGT),
    ...
    if not opts.refilter:
        coverage = 1.0 * opts.fragment_size / opts.step_size
        set1_alnfrc = sum(filt[0]) / coverage / gsize[0]
        set2_alnfrc = sum(filt[1]) / coverage / gsize[1]
        bidirectional_alnfrc = (
            sum(filt[2]) / coverage / gsize[0],
        ...
        sys.stderr.write(
            "WARNING: Total fragment counts, which are the "
